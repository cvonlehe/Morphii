...
class GlobalColors: NSObject {
    ...
    class var lightModeTextColor: UIColor { get { return UIColor ( red: 0.2, green: 0.2235, blue: 0.2902, alpha: 1.0 ) }}
...
//                self.viewToModel.removeAll(keepCapacity: true)
    ...
    func layoutCharacterWithSidesRow(row: [Key], frame: CGRect, isLandscape: Bool, keyWidth: CGFloat, keyGap: CGFloat) -> [CGRect] {
        ...
        for (k, key) in row.enumerate() {
            if k == 0 {
                ...
                frames.append(CGRectMake(rounded(currentOrigin) - 0.5, frame.origin.y, specialCharacterWidth, frame.height))
    ...
    func layoutSpecialKeysRow(row: [Key], keyWidth: CGFloat, gapWidth: CGFloat, leftSideRatio: CGFloat, rightSideRatio: CGFloat, micButtonRatio: CGFloat, isLandscape: Bool, frame: CGRect) -> [CGRect] {
        ...
        var leftButtonWidth = (leftSideAreaWidth - (gapWidth * CGFloat(2 - 1)))
       ...
       // var spaceWidth = UIScreen.mainScreen().bounds.size.width - leftSideAreaWidth * 2 - 30
        ...
        var spaceWidth = 6 + frame.width - leftSideAreaWidth - rightSideAreaWidth - gapWidth * CGFloat(2)
